...
for szamla in szamlak:
    hatarido = date.fromisoformat(szamla["hatarido"])
    if szamla["kifizetve"] == False and hatarido < ma:
        email = szamla["email"]
        if email not in partnerek:
            partnerek[email] = 0
        partnerek[email] += szamla["osszeg"]

for email, osszesen in partnerek.items():
    print(email, osszesen)